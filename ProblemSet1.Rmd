---
title: "ProblemSet1"
output: html_document
date: "2025-11-16"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```



```{r}
library(tidyverse)
library(haven)
library(broom)
library(dplyr)
```

# Problem 3

```{r}
participant <- read_dta("https://reifjulian.github.io/illinois-wellness-data/data/stata/participation.dta")
head(participant)
```

```{r}
# count of employees in control group versus treatment group
treat_counts <- table(participant$treat)
names(treat_counts) <- c("Control Group", "Treatment Group")
print(treat_counts)
```
```{r}
# percentage of those participating in initial screening
participation_rate <- round(mean(participant$screening2016_c[participant$treat == 1], 
                                 na.rm = TRUE), 4)

# count of those participating in initial screening
n_participated <- sum(participant$screening2016_c[participant$treat == 1], 
                      na.rm = TRUE)

cat("\nAmong treatment group~\n")
cat("Number who participated in 2016 screening:", n_participated, "\n")
cat("Participation rate:", participation_rate * 100)
```

# Problem 4

```{r}
claims <- read_dta("https://reifjulian.github.io/illinois-wellness-data/data/stata/claims.dta")
head(claims)
names(claims)
```


```{r}

# Variables measuring characteristics and outcome before August 2016
baseline_vars <- c(
  "male",
  "age50",
  "age37_49",
  "white",
  "covg_0715_0716",
  "covg_1015_0716",
  "diabetes_1015_0716",
  "hyperlipidemia_1015_0716",
  "hypertension_1015_0716",
  "nonzero_spend_0715_0716",
  "pcp_any_office_1015_0716",
  "pcp_any_visits_1015_0716",
  "pcp_total_office_1015_0716",
  "pcp_total_visits_1015_0716",
  "pos_er_critical_1015_0716",
  "pos_hospital_1015_0716",
  "pos_office_outpatient_1015_0716",
  "spendHosp_0715_0716",
  "spendOff_0715_0716",
  "spendRx_0715_0716",
  "spend_0715_0716"
)

# Creating empty data frame with the four required columns
results_q4 <- data.frame(
  variable = character(),
  control_mean = numeric(),
  treat_mean = numeric(),
  p_value = numeric()
)

# Loop through each variable
for (v in baseline_vars) {
  # Running linear regression
  model <- lm(claims[[v]] ~ claims$treat)
  
  # Extract coefficients from regression
  coef_summary <- summary(model)$coefficients
  
  # Control group mean
  control_mean <- coef_summary[1, 1]
  
  # Treatment group difference
  treat_diff <- coef_summary[2,1]
  
  # Treatment group mean
  treat_mean <- control_mean + treat_diff
  
  # Extract p-values
  p_value <- coef_summary[2, 4]
  
  # Add results to data frame
  results_q4 <- rbind(
    results_q4,
    data.frame(
      variable = v,
      control_mean = control_mean,
      treat_mean = treat_mean,
      p_value = p_value
    )
  )
}

# The results
results_q4
```
  

# Problem 5

```{r, echo=FALSE}

# Outcome variables following first year of randomization
outcomes_1yr <- c(
  "covg_0816_0717",
  "diabetes_0816_0717",
  "hyperlipidemia_0816_0717",
  "hypertension_0816_0717",
  "nonzero_spend_0816_0717",
  "pcp_any_office_0816_0717",
  "pcp_any_visits_0816_0717",
  "pcp_total_office_0816_0717",
  "pcp_total_visits_0816_0717",
  "pos_er_critical_0816_0717",
  "pos_hospital_0816_0717",
  "pos_office_outpatient_0816_0717",
  "spendHosp_0816_0717",
  "spendOff_0816_0717",
  "spendRx_0816_0717",
  "spend_0816_0717"
)

# Creating empty data frame with the three required columns
results_q5 <- data.frame(
  variable = character(),
  diff_no_controls  = character(),
  diff_with_controls = character(),
  stringsAsFactors = FALSE
)

# Loop through each outcome variable
for (v in outcomes_1yr) {

  # Model 1: no controls
  t1 <- tidy(lm(as.formula(paste(v, "~ treat")), data = claims), conf.int = TRUE, conf.level = 0.95)
  beta1 <- t1$estimate[t1$term == "treat"]
  se1 <- t1$std.error[t1$term == "treat"]
  
  # Model 2: with controls
  t2 <- tidy(lm(as.formula(paste(v, "~ treat + male + white + age37_49 + age50")), data = claims), conf.int = TRUE, conf.level = 0.95)
  beta2 <- t2$estimate[t2$term == "treat"]
  se2 <- t2$std.error[t2$term == "treat"]
  # Add results to data frame
  results_q5 <- rbind(
    results_q5,
    data.frame(
      variable = v,
      diff_no_controls  = paste0(round(beta1, 4), " (", round(se1, 4), ")"),
      diff_with_controls = paste0(round(beta2, 4), " (", round(se2, 4), ")"),
      stringsAsFactors = FALSE
    )
  )
}
# The results
results_q5
```

# Question 6

```{r, echo=FALSE}

# Creating empty data frame with the three required columns
results_q6 <- data.frame(
  variable = character(),
  diff_no_controls  = character(),
  diff_with_controls = character(),
  stringsAsFactors = FALSE
)

# Loop through each outcome variable
for (v in outcomes_1yr) {

  # Model 3: no controls
  t3 <- tidy(lm(as.formula(paste(v, "~ hra_c_yr1")), data = claims), conf.int = TRUE, conf.level = 0.95)
  beta3 <- t3$estimate[t3$term == "hra_c_yr1"]
  se3 <- t3$std.error[t3$term == "hra_c_yr1"]

  # Model 4: with controls
  t4  <- tidy(lm(as.formula(paste(v, "~ hra_c_yr1 + male + white + age37_49 + age50")), data = claims), conf.int = TRUE, conf.level = 0.95)
  beta4 <- t4$estimate[t4$term == "hra_c_yr1"]
  se4 <- t4$std.error[t4$term == "hra_c_yr1"]
  
  # Add results to data frame
  results_q6 <- rbind(
    results_q6,
    data.frame(
      variable = v,
      diff_no_controls  = paste0(round(beta3, 4), " (", round(se3, 4), ")"),
      diff_with_controls = paste0(round(beta4, 4), " (", round(se4, 4), ")"),
      stringsAsFactors = FALSE
    )
  )
}
# The results
results_q6
```